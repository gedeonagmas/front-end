...
import { useAdminAuthGetQuery } from "./../../features/api/apiSlice";
...
const AdminAuth = () => {
  ...
  const updateHandler = () => {
    if (data[0].name === newName && data[0].password === newPassword) {
      setAdminPage(true);
      setCorrect(false);
    } else {
      setWrong(true);
  ...
  return (
    <div className="flex flex-col place-items-center pb-44 mt-10">
      {correct && (
        <div className="relative flex flex-col mt-20 gap-y-1 py-8 bg-white h-auto py-[70px]  w-72 items-center rounded-md shadow-lg shadow-black px-4">
